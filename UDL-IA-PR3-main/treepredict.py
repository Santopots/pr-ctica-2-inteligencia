...
def divideset(part: Data, column: int, value) -> Tuple[Data, Data]:
    ...
    for row in part:
        if split_function(row, column, value):
            ...
            set2.append(row)
    return (set1, set2)
    # divideix un set de dades a una columna especifica
...
def buildtree(part: Data, scoref=entropy, beta=0):  # Recursive version
    ...
    """
    if len(part) == 0:  # part son les dades que s'estan processant
        return decision_node.DecisionNode()  # si no hi han dades, retorna un node de decisio buit
    ...
    # es crea un diccionari per a rastrejar els valors únics de cada característica
    for col in range(0, column_count):
        ...
        for value in column_values.keys():
            ...
            if gain > best_gain and len(set1) > 0 and len(set2) > 0:
                ...
                best_sets = (set1, set2)
                # es calcula el guany d informacio per la divisió actual per saber si es worth el split
    if best_gain > beta:
    ...
    # retorna el millor criteri de divisió trobat
...
def iterative_buildtree(part: Data, scoref=entropy, beta=0):
    ...
    while not stack.is_empty():
        ...
        if level == 0:
            ...
            if current_score == 0:
                node_stack.push(DecisionNode(results=unique_counts(data), split_quality=0))
            else:
                best_gain = 0.0
                best_criteria = None
                best_sets = None
...
def classify(tree, row):
    ...
    if isinstance(tree.value, (int, float)):
        ...
        if _split_categorical(row, tree.col, tree.value):
            ...
            return classify(tree.fb, row)
    # Si tree.results no és None, significa que el node
    # actual és una fulla de l'arbre i conté els recomptes de les etiquetes de les mostres que han arribat a aquesta fulla
    # utilitza un arbre de decisió per classificar una fila de dades en una categoria.
    # Es mou a través de l'arbre basant-se en les característiques
    # de la fila i les decisions a cada node, fins que arriba a una fulla
    # i retorna l'etiqueta més comuna en aquesta fulla
